...
import { BOARD_SIZE, CELL_SIZE, CELL_MARGIN } from "./board.constants";

const LUMA_WEIGHTS = [0.2126, 0.7152, 0.0722];

export function luma(data: ArrayLike<number>, at: number): number {
  return (
    data[at] * LUMA_WEIGHTS[0] +
    data[at + 1] * LUMA_WEIGHTS[1] +
    data[at + 2] * LUMA_WEIGHTS[2]
  );
}
...
export function boardTransform(
  ...
  if (crosses.some((cross) => cross <= 0)) {
    throw new Error(
      "Choose corners clockwise: top left, top right, bottom right, bottom left.",
    );
  }
  const horizontal = {
  ...
  if (
    horizontal.x <= Math.abs(horizontal.y) ||
    vertical.y <= Math.abs(vertical.x)
  ) {
    throw new Error(
      "Keep the board upright in the picture. Select top left first, then top right, bottom right, and bottom left.",
    );
  }
  const area =
    Math.abs(
  ...
  if (
    ...
    throw new Error(
      "Move closer so the whole board fills more of the picture.",
    );
  }
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      ...
      const sourceX = Math.max(
        0,
        Math.min(width - 1.001, (a * u + b * v + c) / denominator),
      );
      const sourceY = Math.max(
        0,
        Math.min(height - 1.001, (d * u + e * v + f) / denominator),
      );
      ...
      for (let channel = 0; channel < 3; channel++) {
        const topValue =
          data[at + channel] * (1 - dx) + data[at + 4 + channel] * dx;
        const bottomValue =
          data[at + width * 4 + channel] * (1 - dx) +
          data[at + width * 4 + 4 + channel] * dx;
        value +=
          (topValue * (1 - dy) + bottomValue * dy) * LUMA_WEIGHTS[channel];
      }
...
export function normalize(
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const row = Math.max(
        0,
        Math.min(2, Math.floor((y - padding) / CELL_SIZE)),
      );
      const col = Math.max(
        0,
        Math.min(2, Math.floor((x - padding) / CELL_SIZE)),
      );
      output[y * size + x] = Math.max(
        0,
        1 - pixels[y * size + x] / Math.max(1, paper[row * 3 + col]),
      );
    }
  }
  return output;
...
export function luminance(image: ImageData, x: number, y: number): number {
  ...
  return (
    (value(at) * (1 - dx) + value(at + 4) * dx) * (1 - dy) +
    (value(at + image.width * 4) * (1 - dx) +
      value(at + image.width * 4 + 4) * dx) *
      dy
  );
}
...
export function validateImage(image: ImageData): void {
  if (
    ...
    throw new Error(
      "The camera picture is not ready. Wait for a clear live image and try again.",
    );
  }
}
