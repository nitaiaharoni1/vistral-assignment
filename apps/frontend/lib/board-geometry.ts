...
export function quadrilateralTransform(points: Point[]): number[] {
  ...
  for (let column = 0; column < 8; column++) {
    ...
    if (Math.abs(divisor) < 1e-8)
      throw new Error(
        "Spread the four corners around the whole board and try again.",
      );
    for (let index = column; index < 9; index++)
      equations[column][index] /= divisor;
...
export function projectUv(transform: number[], u: number, v: number): number[] {
  ...
  return [
    (transform[0] * u + transform[1] * v + transform[2]) / w,
    (transform[3] * u + transform[4] * v + transform[5]) / w,
  ];
}
