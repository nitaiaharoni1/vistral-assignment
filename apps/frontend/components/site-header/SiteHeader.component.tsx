...
const headerButtonClass = cn(
  "inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-3.5 py-[9px] text-sm text-[#555c6a] hover:bg-[#eaedf3] hover:text-ink",
  "max-[600px]:gap-1.5 max-[600px]:p-2 max-[600px]:text-[13px]",
);
...
function headerClass(compact: boolean): string {
  return cn(
    ...
    compact
      ? "h-16"
      : "h-[88px] min-[1600px]:h-[104px] max-[800px]:h-[76px] max-[600px]:h-16",
...
const brandMarkClass = cn(
  "mr-0.5 flex items-center gap-px text-accent",
  "max-[600px]:gap-0 max-[600px]:[&_svg]:w-[18px] max-[359px]:hidden",
);
  ...
  return (
    <header className={headerClass(compact)}>
      ...
      >
        ...
        paperplay
        <span className="ml-[3px] text-xs font-medium tracking-normal text-muted max-[800px]:hidden">
          by Vistral
        </span>
      </a>
      <div className="flex items-center gap-1">
        ...
        >
          {voiceOn ? (
            ...
            <SpeakerSlash size={19} aria-hidden="true" />
          )}
