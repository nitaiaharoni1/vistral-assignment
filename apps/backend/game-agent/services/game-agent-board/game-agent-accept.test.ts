...
const READER = { model: "test-model", promptVersion: "paper-board-v2" };
...
describe("preservePendingMove", () => {
  it("keeps the previous O when the board did not change", () => {
    ...
    expect(board.preservePendingMove(next, previous).pendingMove).toBe(0);
  });

  it("does not keep a stale O after the board changes", () => {
    const previous = session({ board: marks("....X...."), pendingMove: 0 });
    const next = session({ board: marks("X...O...."), pendingMove: 8 });
    expect(board.preservePendingMove(next, previous).pendingMove).toBe(8);
  });
});
...
describe("acceptAnalysis", () => {
  it("holds an unclear reading without changing the session", () => {
    ...
    const unclear = board.acceptAnalysis(game, analysis("????.....", { clear: false }), READER);
  ...
  it("holds a low-confidence cell as uncertain", () => {
    ...
    expect(board.acceptAnalysis(game, reading, READER).status).toBe("uncertain");
  });

  it("rejects an illegal clear board", () => {
    const game = savedGame();
    const next = board.acceptAnalysis(game, analysis("XX......."), READER);
  ...
  it("imports the first legal board and lets minimax choose O", () => {
    const game = savedGame();
    const next = board.acceptAnalysis(game, analysis("....X...."), READER);
  ...
  it("accepts O drawn in a different empty square and explains it", () => {
    ...
    const drawn = current.board.findIndex((mark, cell) => mark === null && cell !== intended);
    const key = current.board.map((mark, cell) => (cell === drawn ? "O" : (mark ?? "."))).join("");
    const game = gameWithSession(current);
    const next = board.acceptAnalysis(game, analysis(key), READER);
    expect(next.status).toBe("ready");
    expect(next.notice).toMatch(/O accepted/);
    expect(next.session.board[drawn]).toBe("O");
    expect(next.session.events.some((event) => event.kind === "ai-placement-changed")).toBe(true);
  });

  it("rejects extra marks after the game is already initialized", () => {
    const game = gameWithSession(resumeBoard(marks("X........")));
    const next = board.acceptAnalysis(game, analysis("XXX......"), READER);
    expect(next.status).toBe("rejected");
  });

  it("imports a finished first board without asking for O", () => {
    const game = savedGame();
    const next = board.acceptAnalysis(game, analysis("XXXOO...."), READER);
    expect(next.status).toBe("ready");
