...
```mermaid
flowchart TB
  Camera["Camera frames"] --> Worker["Browser worker: find and track grid"]
  Worker --> Client["Frontend: select a stable 384x384 crop"]
  Client --> API["NestJS: validate request and manage game"]
...
**Built: selecting an image.** The browser worker targets 16 analysis frames/second and finds, tracks, or reacquires the grid. `GameAgentStore` separately samples good-quality views at most once per 120 ms, creates a **384×384 JPEG** at quality 0.85, and measures ink changes. Automatic submission compares unoccupied cells with the last four submitted signatures and requires at least three stable samples over the game's **300–900 ms** window. The local worker's own rectified board is 288×288. These image paths have different jobs; a temporary worker snapshot is not persisted learning.

**Built: reading and deciding.** `GameAgentAnalyzeService` calls `@openrouter/sdk` with the configured default `google/gemini-3.8-flash`, prompt `paper-board-v2`, temperature 0, a strict JSON schema, and a 20-second timeout. SDK retries are disabled. The model receives the crop, saved board/pending O as context, and up to two corrected images. It returns **nine cells, confidence per cell, a clear flag, and a reason**. It does not return a move. The prompt treats text inside images as data and asks the reader to use photographed ink rather than infer marks from turn order.
...
sequenceDiagram
  ...
  Browser->>Server: JPEG, session ID, revision, request ID
...
The starting window is **300 ms**. A game is eligible to change it only after **at least six automatic checks and three accepted added marks**, with **no manual corrections, rejected readings, or request failures**:
...
Each session keeps its starting timing. Completion is recorded once; abandoned games do not update the policy. The key combines browser profile, model, `paper-board-v2`, and `capture-timing-v1`. `captureProfiles` stores the latest setting/version and last summary; each saved game's `reply.feedback` retains its metrics. **Save session log** exports these as `captureFeedback`, including the starting setting/version, next setting, completion time, and update reason.
...
**Validation for this documentation update.** The current checkout passes `npm run build`. Nest application initialization and service injection were also checked with an isolated storage directory and no API key. No model call, test suite, new physical-camera game, or feedback-improvement evaluation was run for this update.

**Built traceability.** Accepted `agent-analysis` events record model name, `paper-board-v2`, board, and decision source (`rules` or `saved`). `agent-request` records provider request ID, automatic/manual trigger, shortened snapshot hash, latency, cost, and outcome. `capture-feedback` explains timing updates. Session exports label camera perception `openrouter-board-v1`, local perception `classical-ink-v1`, rules `tic-tac-toe-v1`, and policy **`minimax-v1` in both modes**. These are code labels, not hashes of every artifact. Only the latest analyzed image/response is retained, so old decisions cannot all be reconstructed from complete inputs.
