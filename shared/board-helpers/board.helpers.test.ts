...
import { addedCells } from "./board.helpers";
import { checkLegality } from "./board.helpers";
import { chooseMove } from "./board.helpers";
import { getOutcome } from "./board.helpers";
import { phaseForBoard } from "./board.helpers";
import { sameBoard } from "./board.helpers";
...
describe("checkLegality", () => {
  ...
  it("rejects the wrong length or an unknown mark", () => {
    ...
    expect(checkLegality(["Z", ...Array(8).fill(null)] as never).valid).toBe(false);
  });
