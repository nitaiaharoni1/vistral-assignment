...
import { acceptStableBoard, confirmMove, resumeBoard } from "./accept-session";
import { createSession } from "./session.helpers";
import { keyOf, marks, session } from "../tests/helpers/boards.ts";
...
describe("confirmMove", () => {
  it("confirms X and asks for O in live play", () => {
    const board = marks("....X....");
    const next = confirmMove(createSession(), board, 4, 100);
  ...
  it("closes the game when the confirmed mark wins", () => {
    ...
    const next = confirmMove(previous, marks("XXXOO...."), 2, 50);
    expect(next.phase).toBe("finished");
  ...
  it("only observes marks in replay, without choosing O", () => {
    ...
    const next = confirmMove(previous, marks("....X...."), 4, 10);
    expect(next.phase).toBe("draw-ai");
    expect(next.pendingMove).toBeNull();
...
describe("acceptStableBoard", () => {
  ...
  it("rejects extra live marks and a misplaced O", () => {
    ...
    const other = wrongO.findIndex(
      (mark, cell) => mark === null && cell !== afterX.pendingMove,
    );
    wrongO[other] = "O";
  ...
  it("syncs several legal replay additions and rejects illegal ones", () => {
    ...
    expect(synced.events.some((event) => event.kind === "board-sync")).toBe(
      true,
    );
  ...
  it("clears a recovery when the paper matches again", () => {
    ...
    expect(recovered.events.some((event) => event.kind === "recovered")).toBe(
      true,
    );
  });
});
