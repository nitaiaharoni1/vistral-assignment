...
import { addEvent } from "./session.helpers";
import { cellLabel } from "./session.helpers";
import { cellVisible } from "./session.helpers";
import { checkVisibleChange } from "./session.helpers";
import { createSession } from "./session.helpers";
import { damagedCellMessage } from "./session.helpers";
import { expectedMark } from "./session.helpers";
import { hasPendingMove } from "./session.helpers";
import { instruction } from "./session.helpers";
import { isUnexpectedMark } from "./session.helpers";
import { pauseSession } from "./session.helpers";
import { rejectChange } from "./session.helpers";
import { resetStability } from "./session.helpers";
import { resumeSession } from "./session.helpers";
import { stableCell } from "./session.helpers";
...
describe("session messages", () => {
  ...
  it("flags the wrong mark and a misplaced pending O", () => {
    ...
    expect(isUnexpectedMark(session({ phase: "draw-ai", pendingMove: 4 }), "O", 0)).toBe(true);
    expect(isUnexpectedMark(session({ phase: "draw-ai", pendingMove: 4 }), "O", 4)).toBe(false);
  });
  ...
  it("explains a damaged confirmed cell", () => {
    ...
    expect(damagedCellMessage(session({ mode: "replay", board: play.board }), 0)).toMatch(/recording/);
  });
});

describe("instruction", () => {
  it("announces a finished live game", () => {
    expect(instruction(session({ phase: "finished", board: marks("XXXOO....") }))).toMatch(/You win/);
    expect(instruction(session({ phase: "finished", board: marks("OOOXX.X..") }))).toMatch(/O wins/);
    expect(instruction(session({ phase: "finished", board: marks("XOXOOXXXO") }))).toMatch(/draw/);
  });
  ...
  it("tells the person where to draw O", () => {
    expect(instruction(session({ phase: "draw-ai", pendingMove: 4 }))).toMatch(/Draw O in center/);
  });
  ...
  it("asks for X on a live turn and watches replay", () => {
    ...
    expect(instruction(session({ mode: "replay", phase: "draw-ai" }))).toMatch(/Watching O/);
  });
});

describe("session lifecycle", () => {
  it("only reports a pending move during live draw-ai", () => {
    expect(hasPendingMove(session({ phase: "draw-ai", pendingMove: 4 }))).toBe(true);
    expect(hasPendingMove(session({ mode: "replay", phase: "draw-ai", pendingMove: 4 }))).toBe(false);
  });
  ...
  it("only rejects a visible change once it is stable", () => {
    const checking = checkVisibleChange({ session: session(), reason: "changed", at: 10, stable: false });
    expect(checking.checkingChange).toBe(true);
    const rejected = checkVisibleChange({ session: session(), reason: "changed", at: 10, stable: true });
  ...
  it("numbers events in order", () => {
    const next = addEvent(session(), { at: 1, kind: "pause", message: "hi" });
...
describe("cellVisible and stableCell", () => {
  it("requires a finite confidence in range and a real mark", () => {
    expect(cellVisible({ mark: "X", confidence: 0.78, ink: 0.1, readable: true })).toBe(true);
    expect(cellVisible({ mark: "?", confidence: 0.99, ink: 0.1, readable: true })).toBe(false);
    expect(cellVisible({ mark: "X", confidence: 0.77, ink: 0.1, readable: true })).toBe(false);
    expect(cellVisible({ mark: "X", confidence: 1.1, ink: 0.1, readable: true })).toBe(false);
  });

  it("needs five frames and 500 ms", () => {
    expect(stableCell(null, 1000)).toBe(false);
    expect(stableCell({ mark: "X", frames: 5, since: 500, lastSeen: 1000 }, 1000)).toBe(true);
    expect(stableCell({ mark: "X", frames: 4, since: 0, lastSeen: 1000 }, 1000)).toBe(false);
    expect(stableCell({ mark: "X", frames: 5, since: 600, lastSeen: 1000 }, 1000)).toBe(false);
  });
});
